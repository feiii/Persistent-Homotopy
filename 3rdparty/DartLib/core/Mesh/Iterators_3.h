...
template <typename M>
//change implementation, as we only care about edges on the boundary.
...
//     {
//         m_pEdge = pE;
//         m_pDart = (typename M::CDart*) m_pEdge->dart();
//     };
...
//     void operator++()
//     {
//         m_pDart = (typename M::CDart*) m_pDart->beta(2);

//         if (m_pDart == (typename M::CDart*) m_pEdge->dart())
//         {
//             m_pDart = NULL;
//             return;
//         }
...
//     typename M::CFace* operator*() { return (typename M::CFace*) m_pDart->cell(2); };

//   protected:
//     typename M::CEdge* m_pEdge;
//     typename M::CDart* m_pDart;
// };
//Changed a bit, pD->face() is not valid, change it to pD->cell(2) indicating the face.
class EdgeFaceIterator
{
  public:
    EdgeFaceIterator(typename M::CEdge* pE)
    {
        ...
        m_faces.insert((typename M::CFace*) pD->cell(2));
  ...
  protected:
    std::set<typename M::CFace*> m_faces;
    typename std::set<typename M::CFace*>::iterator m_iter;
};
